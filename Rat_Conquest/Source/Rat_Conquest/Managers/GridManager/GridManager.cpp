...
FVector AGridManager::GetRandomPositionInGrid()
{
    ...
    for (const auto& TilePair : GridTiles)
    {
        ...
        if (CurrentTile && !CurrentTile->bIsOccupied && !CurrentTile->ItemSlot.IsValid() && !CurrentTile->InteractableObjectSlot.IsValid() && !CurrentTile->unitRefrence.IsValid())  // Ensure the tile exists and is not occupied
        {
